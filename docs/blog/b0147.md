...
```bash
sudo -u postgres psql -c "CREATE USER radius WITH PASSWORD 'mail1234';"
...
sed -i -e 's/dialect = "sqlite"/dialect = "postgresql"/' \
       ...
       -e 's/password = "radpass"/password = "mail1234"/' /etc/freeradius/3.0//mods-available/sql
```
...
# Use system's password database (e.g., PostgreSQL)\n\
auth       required     pam_unix.so use_first_pass" /etc/pam.d/radiusd

sed -n '4,15p' /etc/pam.d/radiusd

echo "\
# Instruct FreeRADIUS to use PAM to authenticate users
DEFAULT 
        Auth-Type := PAM" >> /etc/freeradius/3.0/mods-config/files/authorize
sed -n '207,209p' /etc/freeradius/3.0/mods-config/files/authorize
```
...
```bash
radtest bob mail1234 localhost 0 testing123
```
...
Sent Access-Request Id 98 from 0.0.0.0:53028 to 127.0.0.1:1812 length 73
        User-Name = "bob"
        User-Password = "mail1234"
        NAS-IP-Address = 10.1.1.100
        NAS-Port = 0
        Message-Authenticator = 0x00
        Cleartext-Password = "mail1234"
...
```bash
radtest bob mail1234 10.1.1.100 0 testing123
```
...
```bash
Sent Access-Request Id 125 from 0.0.0.0:40436 to 10.1.81.136:1812 length 73
        User-Name = "bob"
        User-Password = "mail1234"
        ...
        Message-Authenticator = 0x00
        Cleartext-Password = "mail1234"
Received Access-Accept Id 125 from 10.1.81.136:1812 to 10.1.200.1:40436 length 20
```
...
```bash
echo y | su ${user:-bob} -c "google-authenticator -t -d -r 1 -R 30 -s /home/${user:-bob}/.ssh/.google_authenticator"
```
