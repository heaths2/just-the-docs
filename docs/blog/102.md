...
```bash
cp -a /usr/lib/syslinux/modules/bios/ldlinux.c32 /srv/tftp/ubuntu20.04/
cp -a /usr/lib/PXELINUX/pxelinux.0 /srv/tftp/ubuntu20.04/
```
...
LABEL 1
  ...
  KERNEL vmlinuz
  INITRD initrd
  APPEND root=/dev/ram0 ramdisk_size=1500000 ip=dhcp url=http://192.168.0.37/ubuntu20.04/ubuntu-20.04.6-live-server-amd64.iso

LABEL 2
  MENU LABEL ^2) Install Ubuntu 22.04
  KERNEL vmlinuz
  INITRD initrd
  APPEND root=/dev/ram0 ramdisk_size=1500000 ip=dhcp url=http://192.168.0.37/ubuntu22.04/ubuntu-22.04.3-live-server-amd64.iso
EOF
```
