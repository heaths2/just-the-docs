---
layout: default
title: 젠 하이퍼바이저(Xen Hypervisor) 설치
parent: Blog
...
network:
  ...
  bridges:
    xenbr0:
      ...
      interfaces: [eno1]
      #- eno1
      addresses: [10.1.255.211/16]
      gateway4: 10.1.255.1
      nameservers:
        addresses: [164.124.101.2, 1.1.1.1]
  version: 2
```
...
chown \_apt:\_apt xen-tools_4.9.2-1_all.deb
...
![IMG-001](https://user-images.githubusercontent.com/36792594/183539111-abfa702c-24f1-4cd1-9a29-9e37fd4a219e.png)
...
![IMG-002](https://user-images.githubusercontent.com/36792594/183539339-19866ab8-1fe0-492d-adda-f570d23f70dc.png)
...
![IMG-003](https://user-images.githubusercontent.com/36792594/183539341-c710e09e-2172-44e8-a4e7-95f76f730b27.png)
...
![IMG-004](https://user-images.githubusercontent.com/36792594/183536477-b17a8156-50cd-42b2-a377-2954242b4000.png)
...
#GRUB_CMDLINE_LINUX_DEFAULT="maybe-ubiquity"
#GRUB_CMDLINE_LINUX=""
GRUB_CMDLINE_LINUX_DEFAULT="dom0_mem=4096m dom0_max_vcpus=2 dom0_vcpus_pin maybe-ubiquity"
GRUB_CMDLINE_LINUX="dom0_mem=4096m dom0_max_vcpus=2 dom0_vcpus_pin"
```

![IMG-005](https://user-images.githubusercontent.com/36792594/183536475-3a94c39a-e028-4d8c-b3ff-0f65b290eee8.png)
...
lvcreate -L 1G -n ubuntu1_swap vg
lvs
```
...
```bash
# Xen 설정 파일 복사 및 설정
cp -a /etc/xen/xlexample.pvlinux /etc/xen/ubuntu1.cfg
...
vif = [ 'bridge=xenbr0' ]
# Disk Devices
disk = [ 'phy:/dev/vg/ubuntu1_root,xvda1,rw','phy:/dev/vg/ubuntu1_swap,xvdb1,rw' ]
```
...
xl list
# Xen Guest OS 설치
xl create -c /etc/xen/ubuntu1.cfg
```
...
mkdir -p /mnt/ubuntu1/
mount /dev/vg/ubuntu1_root /mnt/ubuntu1/
ls -al /mnt/ubuntu1/
```
...
```bash
# kernel 다운로드
apt-get --download-only install linux-image-5.4.0-80-generic
