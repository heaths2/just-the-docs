...
```bash
sed -i 's/#GRUB_CMDLINE_XEN_DEFAULT=.*/GRUB_CMDLINE_XEN_DEFAULT="dom0_mem=1G,max:4G dom0_max_vcpus=1-2 dom0_vcpus_pin iommu=1 msi=1 pci_pt_e820_access=on watchdog ucode=scan crashkernel=256M,below=4G console=vga vga=mode-0x0311"/' /etc/default/grub.d/xen.cfg
sed -i 's/#GRUB_CMDLINE_XEN=.*/GRUB_CMDLINE_XEN="dom0_mem=1G,max:4G dom0_max_vcpus=1-2 dom0_vcpus_pin iommu=1 msi=1 pci_pt_e820_access=on watchdog ucode=scan crashkernel=256M,below=4G console=vga vga=mode-0x0311"/' /etc/default/grub.d/xen.cfg
```
