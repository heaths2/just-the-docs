...
- `/etc/default/grub.d/xen.cfg` Xen 하이퍼바이저 옵션 설정

```bash
GRUB_CMDLINE_XEN_DEFAULT="dom0_mem=3G,max:16G dom0_max_vcpus=1-2 dom0_vcpus_pin iommu=1 msi=1 pci_pt_e820_access=on watchdog ucode=scan crashkernel=256M,below=4G console=vga vga=mode-0x0311"
GRUB_CMDLINE_XEN="dom0_mem=3G,max:16G dom0_max_vcpus=1-2 dom0_vcpus_pin iommu=1 msi=1 pci_pt_e820_access=on watchdog ucode=scan crashkernel=256M,below=4G console=vga vga=mode-0x0311"
```
