...
#GRUB_CMDLINE_LINUX_DEFAULT="maybe-ubiquity"
#GRUB_CMDLINE_LINUX=""
GRUB_CMDLINE_LINUX_DEFAULT="pci_pt_e820_access=on quiet maybe-ubiquity"
GRUB_CMDLINE_XEN_DEFAULT="dom0_mem=1G,max:1G dom0_max_vcpus=1 dom0_vcpus_pin iommu=1 msi=1"
```

> GRUB_CMDLINE_LINUX_DEFAULT: 일반적인 리눅스 커널 부팅 시에 적용되는 매개변수를 정의합니다. 이 변수는 일반적으로 모든 리눅스 커널 부팅에 적용되는 매개변수를 설정하는 데 사용됩니다.

GRUB_CMDLINE_XEN_DEFAULT: Xen 가상화 환경에서 사용되는 부트 매개변수를 정의합니다. Xen은 독자적인 가상화 기술을 사용하며, 이 변수는 Xen 가상 머신 (domU) 및 Xen 호스트 (dom0)에 대한 부트 매개변수를 설정하는 데 사용됩니다.

GRUB_CMDLINE_LINUX: 일반적인 리눅스 설정을 정의하는 매개변수입니다. 이 변수는 사용자가 추가적인 부트 매개변수를 정의하거나 수정할 수 있는 공간입니다. GRUB_CMDLINE_LINUX_DEFAULT와 유사한 역할을 수행하지만, 사용자 지정 설정에 특화되어 있습니다.
{: .important}
