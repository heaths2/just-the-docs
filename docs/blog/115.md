...
:dhcperror
prompt --key s --timeout 10000 DHCP failed, Hit the ${bold}s${boldoff} key to open iPXE shell && shell || reboot

:netboot
chain tftp://${next-server}/main.ipxe ||
prompt --key s --timeout 10000 Chainloading failed, Hit the ${bold}s${boldoff} key to open iPXE shell && shell || reboot
```
