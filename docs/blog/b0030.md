...
sed -i -e 's/#GRUB_CMDLINE_XEN_DEFAULT=.*/GRUB_CMDLINE_XEN_DEFAULT="dom0_mem=16G,max:32G dom0_max_vcpus=1-4 dom0_vcpus_pin iommu=1 msi=1 pci_pt_e820_access=on watchdog ucode=scan crashkernel=256M,below=4G vga=mode-0x0314 console=com1,vga com1=115200,8n1 loglvl=all guest_loglvl=all"/' \
...
```bash
[drwxr-xr-x]  /etc/xen-tools/skel
└── [drwx------]  /etc/xen-tools/skel/root
    ├── [-rw-r--r--]  /etc/xen-tools/skel/root/.bashrc
    └── [drwx------]  /etc/xen-tools/skel/root/.ssh
        ├── [-rw-r--r--]  /etc/xen-tools/skel/root/.ssh/authorized_keys
        └── [-rw-r--r--]  /etc/xen-tools/skel/root/.ssh/config
```
...
#    ServerAliveInterval 3600  # 클라이언트가 서버에 1시간(3600초)마다 "keep-alive" 메시지를 보냅니다.
#    ServerAliveCountMax 3     # 서버가 응답하지 않으면 클라이언트는 최대 3번 "keep-alive" 메시지를 보낸 후 연결을 종료합니다.
```
...
```bash
[drwxr-xr-x]  /etc/xen-tools/role.d
└── [-rwxr-xr-x]  /etc/xen-tools/role.d/required-packages
```
...
root        = '/dev/xvda2 ro'
disk        = [
                  'file:/img/domains/jammy/disk.img,xvda2,w',
                  'file:/img/domains/jammy/swap.img,xvda1,w',
              ]
...
root        = '/dev/xvda2 ro'
disk        = [
                  'phy:/dev/Disks/jammy-disk,xvda2,w',
                  'phy:/dev/Disks/jammy-swap,xvda1,w',
              ]
