...
```bash
ansible-vault create --vault-password-file=../vault/.secret secret.yml
```

- 기존 파일 평문을 암호화하여 Vault 파일 생성

```bash
ansible-vault encrypt --vault-password-file=../vault/.secret --output=secret.yml secret
```
...
- 주어진 문자열 Vault로 암호화한 문자열로 출력

```bash
ansible-vault encrypt_string --vault-password-file=../vault/.secret 'qwe1234' --name 'ansible_password'
```
