...
> - [칼리코 CNI 설치가이드 문서](https://docs.tigera.io/calico/latest/getting-started/kubernetes/quickstart)
> - Helm 설치방법
> - 
...
**kubelet** **containerd** CRI 소켓 설정

```bash
sudo sed -i '/KUBELET_EXTRA_ARGS=/c\KUBELET_EXTRA_ARGS=--container-runtime=remote --container-runtime-endpoint=unix:///run/containerd/containerd.sock' /etc/default/kubelet
...
kubeadm init --apiserver-advertise-address=192.168.0.240
```
